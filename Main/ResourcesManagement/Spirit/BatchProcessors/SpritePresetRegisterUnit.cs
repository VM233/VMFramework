    ...
    internal sealed class SpritePresetRegisterUnit : GamePrefabRegisterUnit<Sprite>
    {
        ...
        protected override IGamePrefab OnProcessAsset(Sprite sprite)
        {
            if (checkUnique)
            {
                ...
                {
                    UnityEngine.Debug.LogWarning($"Ignore adding {sprite.name} because it is already registered");
                    return null;
                }
            }
            ...
            {
                UnityEngine.Debug.LogWarning($"Sprite Preset with id {id} already exists.");
                return null;
            }
