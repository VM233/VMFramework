...
namespace VMFramework.GameLogicArchitecture
{
    ...
    public sealed partial class GamePrefabSingleWrapper : GamePrefabWrapper, INameOwner
    {
        ...
        public override void InitGamePrefabs(IReadOnlyCollection<IGamePrefab> gamePrefabs)
        {
            if (gamePrefab != null)
            {
                UnityEngine.Debug.LogWarning($"{nameof(GamePrefabSingleWrapper)}({name}) already has a gamePrefab." +
            ...
            if (count > 1)
            {
                UnityEngine.Debug.LogError($"GamePrefabSingleWrapper can only hold one gamePrefab, but {count} were provided.");
            }
